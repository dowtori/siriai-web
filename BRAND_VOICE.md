...
## 11. Korean Naturalization (v3.3)

> 적용: 2026-05-17 · 사용자 피드백 "‘진단 통화’는 부자연스러운 한국어 — 실제 안 쓰는 표현 전부 개선" 반영. 한국어 카피 풀 스캔 후 부자연·영어 직역 표현 일괄 교체.

### 11.1 R12 — 부자연·영어 직역 회피

| 룰 | 정의 |
|---|---|
| **R12 — 자연 한국어 우선** | 한국어 카피는 **실제로 쓰는 표현**으로. 의학적 어조(`진단`), 영어 직역(`일정 옵션`, `리테이너 어드바이저리`), 부자연 어순(`실제 설계 운영 구조`) 회피. 비즈니스 정착어(`의사결정 트리`, `리터러시`, `KPI`) 등 표준 외래어는 예외 — 한국어 표준 사전 또는 업계 표준 우선. |

### 11.2 v3.2 → v3.3 교체 매핑

| 부자연 (Before) | 자연 (After) | 사유 |
|---|---|---|
| 진단 통화 | **사전 미팅** | "진단"은 의학 어조, "diagnosis call" 직역 |
| 진단 워크숍 | **현황 점검 워크숍** | "진단" 동일 |
| 월간 진단·회고 | **월간 점검·회고** | "진단" 동일 |
| 진단 (단독 명사) | **현황 점검** | 의약 어조 회피 |
| 리테이너 어드바이저리 | **정기 자문** | 영어 직역, 발음 부담 |
| 일정 옵션을 보내드립니다 | **가능한 시간을 회신드립니다** | `schedule options` 직역 |
| 진단 통화를 잡습니다 | **사전 미팅을 정합니다** | `통화 잡기` 구어, `정하다`가 자연 |
| 실제 설계 운영 구조의 추상화 | **실제 운영 구조의 추상화** | "설계/운영" 중복 어색 |
| 진단 통화 응대 목적의 | **사전 미팅 안내를 위한** | "응대 목적"이 너무 사무적 |
| 진단 통화 신청 / Start a diagnosis (CTA) | **메모 보내기 / Send a note** | 메인 §06의 "메모 남기기" 진입 정체성과 일관 |
| 진단 통화 신청이 접수되었습니다 | **메시지가 도착했습니다** | 듀얼 진입(메모 trail)과 일관, 더 자연 |
| 화상 통화 (FAQ) | **화상 미팅** | "사전 미팅"과 통일 |
| 통화 전후로 자료 제출을 요청 | **미팅 전후로 자료를 요청** | "통화" 통일, "제출 요청"이 사무적 |

### 11.3 유지 (예외) — 비즈니스 표준어

| 표현 | 사유 |
|---|---|
| 의사결정 트리 / 구조 | 비즈니스·엔지니어링 표준 |
| 운영 구조 / 의사결정 구조 | brand baseline |
| 마찰 지점 | "friction points" 한국어 정착 |
| 리터러시 / 어드바이저리 / 스튜디오 | 서비스 이름 (외래어 baseline) |
| 트랙별 / 사내 매뉴얼화 | 교육·운영 표준어 |
| KPI | 비즈니스 표준 |
| 임원 1:1 세션 | 표준 |

### 11.4 검수 체크리스트 (§8·§10.5 누적)

- [ ] 의학·법률 등 도메인 외 어조 단어가 brand 카피에 침범 안 했는가 (R12)
- [ ] 영어 직역 어구(`schedule options`, `start a diagnosis` 류) 0건
- [ ] 동일 의미 표현이 페이지·섹션 간 일관 (예: 사전 미팅 ↔ 화상 미팅 ↔ 통화 혼용 0)
- [ ] 어순이 자연 (수식·중심어 순서, 중복 명사 회피)
